...
function createForm(){
    ...
    input.addEventListener('keyup', function(e){
        hintWrap.innerHTML = '';
        hintWrap.classList.add('active'); 
        let inputWord = document.querySelector('.form-input').value;
        let result = new Array();
        for (let i=0; i < arrWord.length; i++){
            if (arrWord[i].includes(inputWord))
                result.push(arrWord[i]);
        }
        if ((inputWord === '') || (inputWord === ' ')){
            hintWrap.innerHTML = '';
            hintWrap.classList.remove('active'); 
        } else{
            drowResults(result,hintWrap); 
        }
    ...
    btn.addEventListener('click', function(e){
        ...
        let searchWord = document.querySelector('.form-input').value;
        let url = `https://www.google.com/search?q=${searchWord}&btnI`;
        open(url);
    });

    let favoriteWords = document.createElement('div');
    favoriteWords.className = 'result-area';

    form.appendChild(input);
    form.appendChild(btn);
    form.appendChild(favoriteWords);
    form.appendChild(hintWrap);

    document.body.appendChild(form);
}

function drowResults(result, hintWrap){
    for (let i=0; i < result.length; i++){
        let hint = document.createElement('div');
        hint.className = 'hint';
        hint.textContent = result[i];
        hint.addEventListener('dblclick', function(e){
            let favoriteItem = document.createElement('p');
            favoriteItem.className = 'favorite-item';
            favoriteItem.textContent = result[i];
            document.querySelector('.result-area').appendChild(favoriteItem);
        });
